...
AgentCore resources use `hashicorp/awscc` 1.102.0:
...
Runtime maps to `awscc_bedrockagentcore_runtime`. AWSCC calls Cloud Control
`CreateResource`, `GetResource`, `UpdateResource`, and `DeleteResource`, with
asynchronous request-status polling. CloudFormation resource schemas and handlers
translate these operations into the service. The result is an ordinary AgentCore
resource with a normal ARN and normal SDK invocation. **No CloudFormation stack
is created.** Supporting IAM and S3 resources use `hashicorp/aws` 6.65.0.

AWSCC offers typed Terraform plans and a close mapping to CloudFormation fields.
Its generated provider release and underlying handlers can lag the service, so
each shape still needs qualification. This version supports three-day Memory
retention. Generic `aws_cloudcontrolapi_resource` is deliberately not used: its
computed-property refresh does not offer the same desired-state drift repair.
