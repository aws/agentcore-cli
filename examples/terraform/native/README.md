# AgentCore with native AWS Terraform resources
...
`aws_bedrockagentcore_agent_runtime` and `aws_bedrockagentcore_memory` use the native AWS provider and AgentCore service APIs.
...
infrastructure; the runtime depends on that infrastructure.
...
- `variables.tf` defines the inputs, including descriptions, tags, environment
  variables, and Memory retention (7-365 days for this provider).
