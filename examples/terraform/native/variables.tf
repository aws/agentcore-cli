...
variable "memory_retention_days" {
  description = "Short-term Memory retention in days; this provider accepts 7-365."
  type        = number
  default     = 7
  validation {
    condition     = var.memory_retention_days >= 7 && var.memory_retention_days <= 365 && floor(var.memory_retention_days) == var.memory_retention_days
    error_message = "Memory retention must be an integer from 7 to 365 days."
  }
}
