...
export function mapHarnessToExportConfig(
  ...
  if (pathSkills.length > 0 && buildType === 'CodeZip') {
    context.exportNotes.push({
      ...
      message:
        `The following skill paths must exist on the container filesystem at runtime: ${pathSkills.map(s => ('path' in s ? s.path : '')).join(', ')}. ` +
...
function buildAgentEnvSpec(
  ...
  return {
    ...
    ...(spec.containerUri && { baseImageEcrArn: ecrArnFromUri(spec.containerUri, context.region) }),
  };
}
...
function resolveModelProvider(provider: 'bedrock' | 'open_ai' | 'gemini'): ModelProvider {
  switch (provider) {
    case 'bedrock':
      return 'Bedrock';
    case 'open_ai':
...
function isPathSkill(skill: HarnessSkill): boolean {
  return 'path' in skill;
}

function isS3Skill(skill: HarnessSkill): boolean {
  return 's3' in skill;
}

function isGitSkill(skill: HarnessSkill): boolean {
  return 'git' in skill;
}
