...
export async function runRecommendationCommand(
  ...
  try {
    ...
    if (options.type === 'SYSTEM_PROMPT_RECOMMENDATION' && evaluatorIds.length !== 1) {
      return {
        success: false,
        error: new Error('System prompt recommendations require exactly one evaluator.'),
        logFilePath: logger?.logFilePath,
      };
    }
    ...
    if (
      ...
      return {
        success: false,
        error: new Error('System prompt content is required. Provide via --inline, --prompt-file, or --bundle-name.'),
        logFilePath: logger?.logFilePath,
      };
    }
