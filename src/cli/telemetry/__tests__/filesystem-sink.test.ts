...
describe('FileSystemSink', () => {
  ...
  it('writes each record as a JSONL line on disk', async () => {
    ...
    sink.record('command_run', 42, { command_group: 'deploy', command: 'deploy', exit_reason: 'success' });
    await sink.flush();
  ...
  it('appends multiple records as separate lines', async () => {
    const sink = createSink();
    sink.record('command_run', 10, { command_group: 'add', command: 'add.agent' });
    sink.record('command_run', 20, { command_group: 'add', command: 'add.memory' });
    await sink.flush();
  ...
  it('creates output directory if it does not exist', async () => {
    ...
    sink.record('command_run', 1, { command_group: 'status', command: 'status' });
    await sink.flush();
  ...
  it('shutdown logs audit message when records were written', async () => {
    ...
    sink.record('command_run', 99, { command_group: 'invoke', command: 'invoke' });
    await sink.shutdown();
