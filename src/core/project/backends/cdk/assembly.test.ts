...
describe("stackForTarget", () => {
  test("selects by the target tag instead of deriving a stack name", async () => {
    ...
    expect(await stackForTarget(json, directory, "prod")).toBe("nested/stack-id");
  });
  ...
  test("ignores non-stack artifacts", async () => {
    ...
    await expect(stackForTarget(json, directory, "prod")).rejects.toThrow(/defines 0 stack/);
  });
  ...
  test("reports a missing manifest before attempting deployment", async () => {
    ...
    await expect(stackForTarget(json, directory, "prod")).rejects.toThrow(
