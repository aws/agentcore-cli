 ...
 * allowlist, X-Agentcore-Local on POSTs, CORS preflight) and routing, serving
 * the prebuilt SPA plus the /api and /invocations routes the SPA calls.
 */
...
import { apiError, errorMessage, json, parseJsonBody, parseTimeParam } from "./respond";
...
export function createInspectorHandler(deps: InspectorDeps): HttpRequestHandler {
  // JSON-RPC ids for A2A message/stream requests, monotonic per handler.
  let a2aRequestId = 1;
  const nextA2aId = () => a2aRequestId++;
