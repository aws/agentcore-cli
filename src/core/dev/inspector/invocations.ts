...
export async function handleInvocations(
  ...
  if (running.protocol === "A2A") {
    return invokeA2aAgent(running.port, parsed, sessionId, nextA2aId);
  }
  if (running.protocol === "AGUI") {
    return invokeAguiAgent(running.port, parsed, sessionId, userId);
  }
  return invokeHttpAgent(running.port, request.body, sessionId, userId);
}
...
/** Re-frame each agent SSE line through {@link parseSseLine}, like the reference pipeSSETransformed. */
...
/**
 * Extract displayable text from one agent SSE line. Handles plain strings,
 * `{"text": ...}` events from the bedrock-agentcore runtime, `{"error": ...}`
 * events, and ConverseStream-shaped content deltas.
 */
  ...
  const a2aBody = {
    ...
    params: {
      message: {
        ...
        ...(sessionId !== undefined && { contextId: sessionId }),
      },
    },
  };
