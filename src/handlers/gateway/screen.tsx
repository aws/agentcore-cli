...
export function GatewayBrowseScreen({ ctx, core }: ScreenProps) {
  const opts = coreOptsFromCtx(ctx);
  const navigate = useNavigate();

  return (
    <PaginatedTablePicker
      breadcrumb={["agentcore", "gateway"]}
      description="browse AgentCore Gateways"
      ...
      onSelect={(gatewayId) =>
        navigate(`/agentcore/gateway/browse/${encodeURIComponent(gatewayId)}`)
      }
      onBack={() => navigate("/agentcore")}
