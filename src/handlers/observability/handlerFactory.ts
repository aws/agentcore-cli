...
export class ObservabilityHandlerFactory implements ObservabilityHandlerFactories {
  constructor(
    private readonly client: CoreObservabilityClient,
  ...
  >(config: { resource: ObservableResourceCommand<K, F> }) {
    const flags = [...config.resource.flags, ...logFlags] as const;
        ...
        const writeRecord = (record: LogRecord) => {
          if (json) {
            ...
            this.io.stdout.write(
              `${record.timestamp.toISOString()}  ${record.message.trimEnd()}\n`,
            );
          }
        };

        await withUserCancellation(async (signal) => {
          const target = await config.resource.resolve(parsed, ctx);
          const { resource } = target;
          const options = target.options ?? coreOptsFromCtx(ctx);
          if (searchMode) {
            const records = this.client.searchLogs(
              resource,
              {
                startTimeMs,
                endTimeMs,
          ...
          this.io.stderr.write(
            `Streaming logs for ${resource.kind} ${resource.id}... (Ctrl+C to stop)\n`,
          );
          const records = this.client.tailLogs(resource, { filterPattern }, options, signal);
          for await (const record of records) writeRecord(record);
        });
      },
    });
